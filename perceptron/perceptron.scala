...
val hashingTF = new HashingTF().setInputCol(tokenizer.getOutputCol).setOutputCol("features").setNumFeatures(200)
...
val train = splits(0)
val test = splits(1)
val layers = Array[Int](200, 400, 100, 50, 10, 2)
val trainer = new MultilayerPerceptronClassifier().setLayers(layers).setBlockSize(128).setSeed(1234L).setMaxIter(1000)
