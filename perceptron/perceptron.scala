...
def main(args: Array[String]) { 
  ...
  val filename = args(0)
  val sparkSession = SparkSession.builder().master("local").appName("Multilayer Perceptron").config("spark.some.config.option", "some-value").getOrCreate()
  ...
  val train = splits(0)
  val test = splits(1)
  val layers = Array[Int](246, 500, 200, 70, 35, 10, 2)
